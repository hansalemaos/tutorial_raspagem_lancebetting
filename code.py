...
while True:
    try:
        ...
        # versão corrigida (ao vivo)
        df.loc[(df.aa_classList == 'KambiBC-sandwich-filter__event-list-item' )]['aa_innerText'].str.split(r'[\r\n]+', expand=False).apply(lambda q:q[:2]+q[3:] if ':' in q[0] else q).apply(pd.Series)[range(7)].rename( columns={0: 'dia', 1: 'hora', 2: 'team1_nome', 3: 'team2_nome', 4: 'team1', 5: 'empate', 6: 'team2'}).assign(team1=lambda q: q['team1'].str.replace(',', '.'), team2=lambda q: q['team2'].str.replace(',', '.'), empate=lambda q: q['empate'].str.replace(',', '.')).astype({'team1': 'Float64', 'empate': 'Float64', 'team2': 'Float64'}) 
        break
